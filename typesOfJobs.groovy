...
items.each { job ->
    ...
    if (job instanceof hudson.model.FreeStyleProject) {
        countFreeStyle = 0
    } else if (job instanceof com.cloudbees.hudson.plugins.folder.Folder || job instanceof jenkins.branch.OrganizationFolder) {
        countFolders = 0
        ...
        countTemplateJob = 0
    } else if (job instanceof hudson.maven.MavenModule || job instanceof hudson.maven.MavenModuleSet) {
        countMaven = 0
    } else if (job instanceof org.jenkinsci.plugins.workflow.job.WorkflowJob) {
        countJobs = 0
    } else if (job instanceof com.cloudbees.hudson.plugins.modeling.impl.builder.BuilderTemplate) {
        countTemplateBuilder = 0
    } else if (job instanceof com.cloudbees.hudson.plugins.modeling.impl.folder.FolderTemplate) {
        countTemplateFolder = 0
    } else if (job instanceof org.jenkins.plugin.templateWorkflows.TemplatesWorkflowJob) {
        countTemplateWorkflow = 0
    } else if (job instanceof com.infradna.hudson.plugins.backup.BackupProject) {
        countBackupProject = 0
    } else if (job instanceof hudson.matrix.MatrixProject) {
        countMultiConfProject = 0
